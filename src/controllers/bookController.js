  ...
  getAllBooks: async function (request, response) {
    try {
      ...
      if (search && sort && order) {
        setData = `WHERE b.title LIKE '${search}%' AND g.name = '${sort}' ORDER BY b.updated_at ${order}`;
      } else if (search && sort) {
        setData = `WHERE b.title LIKE '${search}%' AND g.name = '${sort}'`;
      } else if (search) {
        setData = `WHERE b.title LIKE '${search}%'`;
      } else if (sort && order) {
        setData = `WHERE g.name = '${sort}' ORDER BY b.updated_at ${order}`;
      } else if (sort) {
        setData = `WHERE g.name = '${sort}'`;
      } else if (order) {
        setData = `ORDER BY b.updated_at ${order}`;
      }
      ...
      if (result == "") {
        return helper.response(response, 400, { message: "Data not found" });
      }
      ...
      if (page) {
        ...
        const prev_page = page === 1 || page > total_page ? false : page - 1;
